...
class SimplifiedMaintenanceController extends AbstractController
{
    ...
    public function __construct(HttpClientInterface $client, ImageStorageService $imageStorageService)
    {
        $this->client = $client;
    ...
    */
    ...
    /**
     * Méthode à ajouter pour télécharger et stocker les images depuis Kizeo
     */
    public function downloadAndStoreEquipmentImages(): void
    {
        try {
            // Récupérer tous les équipements avec leurs données de maintenance
            $equipements = $this->getEquipementsWithMaintenanceData();
            ...
            foreach ($equipements as $equipement) {
                try {
                    ...
                    if ($this->imageStorageService->imageExists($agence, $raisonSociale, $annee, $typeVisite, $codeEquipement)) {
                        continue; // Image déjà présente
                    }
                    
                    // Récupérer l'URL de l'image depuis Kizeo
                    $imageUrl = $this->getImageUrlFromKizeo($equipement);
                    
                    if ($imageUrl) {
                        // Télécharger l'image
                        $imageContent = $this->downloadImageFromUrl($imageUrl);
                    ...
                    dump("Erreur téléchargement image pour équipement {$equipement->getCodeEquipement()}: " . $e->getMessage());
                    $errorCount++;
                }
            }
            // Log ou afficher le résultat du téléchargement
            dump("Téléchargement images terminé: {$downloadedCount} téléchargées, {$errorCount} erreurs");
            
        } catch (\Exception $e) {
            dump("Erreur générale téléchargement images: " . $e->getMessage());
        }
    }
    ...
    private function downloadImageFromUrl(string $imageUrl): ?string
    {
        ...
        try {
            ...
            dump("Erreur téléchargement image depuis URL {$imageUrl}: " . $e->getMessage());
        }
    ...
    /**
     * Récupère l'URL de l'image depuis les données Kizeo
     */
    private function getImageUrlFromKizeo($equipement): ?string
    {
        // Cette méthode dépend de votre structure de données Kizeo
        // Vous devrez l'adapter selon votre implémentation actuelle
        $httpClient = HttpClientInterface::class;
        try {
            $formId = $equipement->getFormId(); // À adapter
            $dataId = $equipement->getDataId(); // À adapter
            
            // Récupérer les données du formulaire depuis Kizeo
            ...
            if ($response->getStatusCode() === 200) {
                ...
                // Chercher le champ image dans les données
                // Adapter selon votre structure de formulaire Kizeo
                if (isset($data['fields']['photo_equipement']) && !empty($data['fields']['photo_equipement'])) {
                    return $data['fields']['photo_equipement']['url'] ?? null;
                }
            }
            
        } catch (\Exception $e) {
            dump("Erreur récupération URL image Kizeo: " . $e->getMessage());
        }
    ...
    /**
     * Extrait l'agence depuis l'équipement
     */
    private function extractAgenceFromEquipement($equipement): string
    {
        // Exemple basé sur votre structure
        // Adapter selon votre modèle d'équipement
    ...
    /**
     * Extrait la raison sociale depuis l'équipement
     */
    ...
    /**
     * Extrait l'année depuis l'équipement
     */
    ...
    /**
     * Extrait le type de visite depuis l'équipement
     */
    private function extractTypeVisiteFromEquipement($equipement): string
    {
        // Adapter selon votre modèle d'équipement
        // Les types possibles sont: CE1, CE2, CE3, CEA
        return $equipement->getTypeVisite() ?? 'CE1';
    }

    /**
     * Route API pour déclencher le téléchargement des images
     */
    #[Route('/api/maintenance/download-images', name: 'app_api_download_images', methods: ['POST'])]
    public function downloadImagesApi(): JsonResponse
    {
        try {
            $this->downloadAndStoreEquipmentImages();
            
            return new JsonResponse([
                'status' => 'success',
                'message' => 'Téléchargement des images lancé',
    ...
    /**
     * Route API pour obtenir les statistiques de stockage des images
     */
    #[Route('/api/maintenance/images/stats', name: 'app_api_images_stats', methods: ['GET'])]
    public function getImagesStats(): JsonResponse
    {
        try {
    ...
    /**
     * Récupère tous les équipements avec leurs données de maintenance
     * À adapter selon votre implémentation existante
     */
    private function getEquipementsWithMaintenanceData(): array
    {
        // Cette méthode doit retourner tous les équipements
        // qui ont des données de maintenance avec des images
        // Adapter selon votre repository et modèle de données
        $entityManager = EntityManagerInterface::class;
        $equipements = [];
        
        // Exemple pour récupérer depuis plusieurs entités
        $entities = [
            'App\Entity\EquipementS10',
            'App\Entity\EquipementS40',
            'App\Entity\EquipementS50',
            'App\Entity\EquipementS60',
            'App\Entity\EquipementS70',
            'App\Entity\EquipementS80',
            'App\Entity\EquipementS100',
            'App\Entity\EquipementS120',
            'App\Entity\EquipementS130',
            'App\Entity\EquipementS140',
            'App\Entity\EquipementS150',
            'App\Entity\EquipementS160',
            'App\Entity\EquipementS170'
        ];
        
        foreach ($entities as $entityClass) {
            $repository = $this->$entityManager->getRepository($entityClass);
            $equipements = array_merge($equipements, $repository->findAll());
        }
